...
lineH.addLayout(lineV1,70)
lineH.addLayout(lineV2,30)
...
def add_note():
    note_name, ok = QInputDialog.getText(win, "add zamitky", "name zamitku: ")
    if ok and note_name != "":
...
def search_tag():
    ...
    if but6.text() == "Шукати замітки по тегу" and tag:
        ...
        but6.setText("CRBYENB gjier")
        list_notes.clear()
        list_tag.clear()
        list_notes.addItems(notes_f)
        print(but6.text())
    elif but6.text() == "ckunytu pouk":
        text.clear()
        list_notes.clear()
        list_tag.clear()
...
but3.clicked.connect(self_note)
